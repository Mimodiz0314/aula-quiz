...
// Este módulo es el motor de NUBE: contiene el código original que vivía en
// sessionService.js, movido aquí sin cambios de lógica (solo se ajustaron las
// rutas relativas porque el archivo bajó un nivel a /backends).
//
// sessionService.js ahora es un despachador delgado que delega en este motor
// (o en localBackend) manteniendo idénticas las firmas públicas.
//
...
// Marcador para que el despachador distinga este motor del local.
export const __esLocal = false;
...
/**
 * Archiva un historial YA construido (proveniente de una clase realizada
 * offline) directamente en /historial/{uid}. Aditivo: no altera el flujo de
 * cerrarSesion. Idempotente: si ya existe una entrada con el mismo `_opId`,
 * no la duplica. Usa el uid autenticado actual.
 */
export async function guardarHistorialDirecto(entry, opId) {
  const docenteUid = getEffectiveUid();
  if (!docenteUid) throw new Error('Debes estar autenticado para sincronizar.');

  if (opId) {
    // Comprobación de idempotencia: ¿ya subimos esta operación?
    const existentes = await get(ref(db, `historial/${docenteUid}`));
    if (existentes.exists()) {
      const dup = Object.values(existentes.val()).some((e) => e && e._opId === opId);
      if (dup) return; // ya estaba: no duplicar
    }
  }

  const historialRef = push(ref(db, `historial/${docenteUid}`));
  await set(historialRef, { ...entry, _opId: opId || null });
}
